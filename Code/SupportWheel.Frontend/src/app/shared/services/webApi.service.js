...
WebApiService.$inject = ['$resource', 'baseUrlService', '$q', '$rootScope', '$httpBackend', 'appConstants', 'baseUrlService', 'mediatorService', 'hubService'];

class WebApiService {
    
    constructor($resource, baseUrlService) {

        let _cache = {};
        let baseUrl = baseUrlService.getBaseUrl() + "api/";
        ...
        return function (recurso, surrogateKey) {
            ...
            let webService = createResource(recurso);
            ...
            function load(params) {
                ...
                let queryParams = _parseParams(params);
                let applyScope;
            ...
            function save(params, successCB, failedCB) {
                ...
                if ((surrogateKey != null) && (queryParams.hasOwnProperty(surrogateKey))) {
                    var tipoEvento = appConstants.constEventosDb.updated; //Passou a chave primária, então está atualizando um registro
                } else {
                    var tipoEvento = appConstants.constEventosDb.inserted; //Sem chave primária, novo registro
                }
            ...
            function remove(params, successCB, failedCB) {
                ...
                if (!queryParams.hasOwnProperty(surrogateKey)) {
                    ...
                    webService.remove({ id: queryParams[surrogateKey] })
                        .$promise.then(function (data) {
                            mediatorService.sendEvent(recurso, appConstants.constEventosDb.deleted, data, false);
                            deferred.resolve(data);
                        }, function (err) {
                            deferred.reject(err);
                        });
            ...
            function createResource(recurso) {
                ...
                let resource = $resource(baseUrl + recurso + '/:action/:id',
                    {}, {
                ...
                resource.getAll = function (params) {
                    ...
                    let deferred = $q.defer(); //Cria uma promessa
...
export default angular
    .module('app.shared.services.webApi', [ngResource])
